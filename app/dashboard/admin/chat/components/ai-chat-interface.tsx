...
export default function AIChatInterface() {
  ...
  // ⚡ USE HOOK (IMPORTANT FIX)
  const { messages, sendMessage, getMessages } = ChatHook(
    userId,
    receiverId
  );
  ...
  return (
    <div className="mx-auto flex h-full w-full max-w-4xl flex-col items-center justify-center space-y-4 lg:p-4">
      ...
      {/* CHAT AREA */}
      <ChatContainer
        className={cn("relative w-full flex-1 space-y-4 pe-2 pt-10", {
          hidden: !isFirstResponse
        })}
        ref={containerRef}
        scrollToRef={bottomRef}
      ...
      <div className="bg-primary/10 w-full rounded-2xl p-2">
        ...
        >
          ...
          <PromptInputActions className="flex justify-between p-3">
