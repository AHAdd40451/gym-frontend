...
interface MonthlyRevenue {
  month: number; // IMPORTANT: you're using month as number in Date()
  year: number;
  totalRevenue: number;
  ordersRevenue: number;
  subscriptionsRevenue: number;
...
export function EcommerceTotalRevenueCard() {
  ...
  useEffect(() => {
    const fetchMonthlyRevenue = async () => {
      try {
        ...
        // ✅ FIX: normalize API response so it's ALWAYS an array
        const normalizedData = Array.isArray(data)
          ? data
          : data?.monthlyRevenue || data?.data || [];

        setMonthlyRevenue(normalizedData);
      } catch (error) {
        console.error("Error fetching monthly revenue:", error);
        setMonthlyRevenue([]); // fallback safe state
      } finally {
        setLoading(false);
      }
    };
  ...
  const safeMonthlyRevenue = Array.isArray(monthlyRevenue)
    ? monthlyRevenue
    : [];
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Monthly Revenue</CardTitle>
        <CardDescription>
          Click on a month for detailed breakdown
        </CardDescription>
      </CardHeader>

      <CardContent>
        <div className="space-y-3">
          {safeMonthlyRevenue.slice(0, 6).map((month) => (
            <Link
              key={`${month.year}-${month.month}`}
              href={`/dashboard/admin/ecommerce/revenue-details/${month.year}-${month.month}`}
              className="block"
              ...
              <div className="p-3 rounded-lg border hover:bg-gray-50 transition-colors cursor-pointer">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <div className="text-sm text-muted-foreground">
                      Orders: {month.totalOrders} | Delivered:{" "}
                  ...
                  <div className="text-right">
                    <div className="text-lg font-bold text-green-600">
                      $ {Number(month.totalRevenue || 0).toLocaleString()}
                    </div>
